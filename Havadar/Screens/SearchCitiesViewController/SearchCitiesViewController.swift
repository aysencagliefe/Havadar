...
class SearchCitiesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, SearchCitiesDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "CityCell", for: indexPath) as? SearchCitiesTableViewCell
        cell?.searchCityLabel?.text = (searchCitiesList[indexPath.row]?.il ?? "") + " / " + (searchCitiesList[indexPath.row]?.ilce ?? "")
